//
//  ZHSForgotPasswordViewController.m
//  ZHS
//
//  Created by 邢小迪 on 15/12/13.
...
@interface ZHSForgotPasswordViewController ()<UITextFieldDelegate>
{
    BOOL _isShow;
}
...
@implementation ZHSForgotPasswordViewController
...
- (void)viewDidLoad {
    ...
    self.forgotPasswordBtn.layer.cornerRadius = 5.0f;
    
    self.userPhoneTF.clearButtonMode = UITextFieldViewModeAlways;
    self.codeTF.clearButtonMode = UITextFieldViewModeAlways;
    self.forgotPasswordBtn.enabled = NO;
    if (kWidth < 375) {
        self.observeKeyboardChangeFrameNotification = YES;
    }
}
-(void)keyboardWillShow:(NSNotification *)notification{
    if (_isShow == NO&& ([self.firstPasswordTF isFirstResponder] || [self.secondPassword isFirstResponder])) {
...
-(void)keyboardWillHide:(NSNotification *)notification{
    if (_isShow == YES && ([self.firstPasswordTF isFirstResponder] || [self.secondPassword isFirstResponder])) {
        UIViewAnimationCurve curve = [notification.userInfo integerForKey:UIKeyboardAnimationCurveUserInfoKey];
        NSTimeInterval duration = [notification.userInfo doubleForKey:UIKeyboardAnimationDurationUserInfoKey];
        [UIView animateWithDuration:duration delay:0 options:curve << 16 animations:^{
            DLog(@"%f",self.view.centerX);
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField == self.userPhoneTF) {
        ...
        if (s.length < 11) {
            ...
            self.forgotPasswordBtn.enabled = self.codeTF.text.length == 4;
        }
        ...
        if (s.length < 4) {
            ...
            self.forgotPasswordBtn.enabled = self.userPhoneTF.text.length == 11;
        }
    }
...
//#warning --- 审核测试号码 ---
     ...
     */
    [[THRequstManager sharedManager] asynPOST:[NSString stringWithFormat:@"%@/auth/find-my-password",kHeaderURL] parameters:param withCompeletBlock:^(id responseObject, THRequstErrorCode code, NSError *error) {
        if (code == 1) {
        ...
        self.forgotPasswordBtn.enabled = YES;
...
- (IBAction)zhaohuimima:(id)sender {
    if ([self.firstPasswordTF.text isEqualToString: self.secondPassword.text]) {
        __weak typeof(self)tempSelf = self;
        [[THRequstManager sharedManager] asynPOST:[NSString stringWithFormat:@"%@/auth/reset-my-password",kHeaderURL] parameters:@{
...
//            TNUser *user = [TNUser modelWithJsonDicWithoutMap:data];
//            user.token = data[@"token"];
//            user.ID = data[@"customer"][@"id"];
//            user.mobile = data[@"customer"][@"mobile"];
